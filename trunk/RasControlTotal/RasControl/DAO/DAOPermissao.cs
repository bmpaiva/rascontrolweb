...
namespace DAO
{
    ...
    {       
        ...
        public List<Permissao> ConsultarAllPermissaoPerfil(int idPerfil)
        {
            ...
            {
                List<Permissao> lista = new List<Permissao>();
                string sql = GenericaSQL.ConsultarAllPermissaoPerfil(idPerfil);

                SqlDataReader dr = dao.ExecuteReader(CommandType.Text, sql);

                while (dr.Read())
                {
                    int idPermissao = (int)dr["ID_PERMISSAO"];
                    Permissao permissao = ConsultarPermissaoCodigo(idPermissao);    
                    lista.Add(permissao);
                }
                dr.Close();
        ...
        {
            string sql = GenericaSQL.CadastrarPermissaoPerfil(idPerfil, permissao);
            GenericaDAO dao = GenericaDAO.getInstancia();
        ...
        public void DeletePerfilPermissao(int idPerfil,int idPermissao)
        {
            string sql = GenericaSQL.DeletePerfilPermissao(idPerfil, idPermissao);
            GenericaDAO dao = GenericaDAO.getInstancia();  
            dao.ExecuteNonQuery(CommandType.Text, sql);
        }
